...
const webServerSecurityGroup = new aws.ec2.SecurityGroup(
  `${infraConfigResources.idPrefix}-web-server-sg-${$app.stage}`,
  {
...
const auroraServerlessSecurityGroup = new aws.ec2.SecurityGroup(
  `${infraConfigResources.idPrefix}-aurora-server-sg-${$app.stage}`,
  {
    name: `${infraConfigResources.idPrefix}-aurora-serverless-sg-${$app.stage}`,
    vpcId: rdsVpcResources.vpc.id,
...
const elasticacheServerSecurityGroup = new aws.ec2.SecurityGroup(
  `${infraConfigResources.idPrefix}-redis-server-sg-${$app.stage}`,
  {
